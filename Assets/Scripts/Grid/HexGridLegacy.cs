...
[ExecuteInEditMode]
public class HexGridLegacy : MonoBehaviour
{
    private readonly Dictionary<AxialCoord, HexTile> _hexByCoord = new Dictionary<AxialCoord, HexTile>();

    public IEnumerable<HexTile> GetAllHexes() => _hexByCoord.Values;

    public void BuildRuntimeDictionary(Transform container, HexGridDataLegacy hexGridData)
    {
        _hexByCoord.Clear();
        ...
        {
            if (child.TryGetComponent<HexTile>(out var hex))
            {
    ...
    public HexTile GetRandomHex() => _hexByCoord.ElementAt(Random.Range(0, _hexByCoord.Count)).Value;
    public int GetCount() => _hexByCoord.Count;
    public HexTile GetHex(AxialCoord coord) => _hexByCoord.TryGetValue(coord, out var hex) ? hex : null;
